...
- **Self-attributed TCC identity:** The binary re-execs itself with `responsibility_spawnattrs_setdisclaim` so it becomes its own TCC-responsible process, and carries the `com.apple.security.personal-information.reminders` entitlement plus both Reminders usage descriptions in its linked `__TEXT,__info_plist`. This narrows rather than widens the grant: the permission is attached to this signed, notarized binary and shows up under its own name in System Settings, instead of being inherited from — or silently refused because of — whichever host application happens to launch it. It grants no access on its own; the user still has to approve the prompt.
