...
The npm package's postinstall script and the Claude Code plugin's SessionStart hook do download the signed Swift binary from `github.com/high5-ventures/apple-reminders-for-claude/releases/...` on first install (and refuse to install if the signature does not chain to `Developer ID Application: high5 ventures GmbH`). That download is anonymous from our side — we do not run a server and have no log of who downloaded what; GitHub may keep its own access logs per its own privacy policy.
